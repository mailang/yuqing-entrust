...
class ScheduleController extends Controller
{
    ...
    */
    public function  generate()
    {
       ...
       if (DB::table('schedule')->whereDate('starttime',$starttime)->first()) return;
        if($count%2==0)
        {
            ...
            $data=DB::table('schedule')->where('weekday','星期五')->orderByDesc('starttime')->limit(1)->get();//取出前一天的排班表
            if ($data->first())
            {
                $list=\GuzzleHttp\json_decode($data->first()->schedule);
                ...
                foreach ($admins as $admin)
                {
                  if (in_array($admin->realname,$user1))
                    array_push($push2['admins'],$admin->realname);
                  else
                  {
                      if(in_array($admin->realname,$user2))
                          array_push($push1['admins'],$admin->realname);
                      else{
                          if (count($push1['admins'])<$count/2)array_push($push2['admins'],$admin->realname);
                          else array_push($push1['admins'],$admin->realname);
                      }
                  }
                }
            }
            else
            {
                ...
                $push1['admins']=array_slice($all,0,$count/2);
                $push2['admins']=array_slice($all,$count/2,$count);
            }
            ...
            for ($i=0;$i<5;$i++)
            {
                ...
                else
                {
                    $push1["group"]=2;
                    $push2['group']=1;
                    if ($count>2&&$count<8){$a=$this->getrand($splice,$push1,1);$b=$this->getrand($splice1,$push2,1);}
                    else if($count>6) {$a=$this->getrand($splice,$push1,floor($count/2));$b=$this->getrand($splice1,$push2,floor($count/2));}
    ...
    /*得到随机数*/
    public function getrand($splice,$push1,$num=1)
    {
        // Log::info(\GuzzleHttp\json_encode($push1));
         $a= array_rand($push1['admins'],$num);
         if ($num>1){
         for ($i=0;$i<$num;$i++)
         {
             if (count($splice)>0&&in_array($push1['admins'][$a[$i]],$splice)){array_splice($push1['admins'],$a[$i],1);$this->getrand($splice,$push1);}
         }}
        else {if (count($splice)>0&&in_array($push1['admins'][$a],$splice)){array_splice($push1['admins'],$a,1);$this->getrand($splice,$push1);}}
         return $a;
    }
     /*管理员手动修改排班表*/
